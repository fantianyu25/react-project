...
export default class Table extends React.Component {
  ...
  toggleSortOrder(order, column) {
    ...
    this.props.onChange(...this.prepareParamsArguments(assign({}, this.state, newState)));
  }
  ...
  handleFilter = (column, nextFilters) => {
    ...
    this.setState(newState, () => {
      props.onChange(...this.prepareParamsArguments(assign({}, this.state, {
        selectionDirty: false,
        filters,
        pagination,
      })));
  ...
  handlePageChange = (current) => {
    ...
    this.props.onChange(...this.prepareParamsArguments(assign({}, this.state, {
      selectionDirty: false,
      pagination,
    })));
  }
    ...
    if (this.props.rowSelection) {
      ...
      if (this.props.rowSelection.type === 'radio') {
        selectionColumn = {
          ...
          className: 'jgui-table-selection-column',
        };
      } else {
    ...
    return columns.map((originColumn, i) => {
      ...
      if (column.sorter) {
        ...
        if (isSortColumn) {
          ...
          if (sortOrder) {
            column.className += ' jgui-table-column-sort';
          }
        }
        const isAscend = isSortColumn && sortOrder === 'ascend';
        const isDescend = isSortColumn && sortOrder === 'descend';
        sortButton = (
          <div className="jgui-table-column-sorter">
            <span className={`jgui-table-column-sorter-up ${isAscend ? 'on' : 'off'}`}
              title="↑"
              onClick={() => this.toggleSortOrder('ascend', column)}
            >
              <Icon type="caret-up" />
            </span>
            <span className={`jgui-table-column-sorter-down ${isDescend ? 'on' : 'off'}`}
              title="↓"
  ...
  render() {
    ...
    const classString = classNames({
      [`jgui-table-${this.props.size}`]: true,
      'jgui-table-bordered': this.props.bordered,
    });
    ...
    if (!data || data.length === 0) {
      emptyText = (
        <div className="jgui-table-placeholder">
          {locale.emptyText}
        </div>
      );
      emptyClass = 'jgui-table-empty';
    }
    ...
    const paginationPatchClass = (this.hasPagination() && data && data.length !== 0)
            ? 'jgui-table-with-pagination'
            : 'jgui-table-without-pagination';
    const spinClassName = this.props.loading ? `${paginationPatchClass} jgui-table-spin-holder` : '';
