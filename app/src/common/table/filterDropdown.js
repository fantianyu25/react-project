...
export default class FilterMenu extends React.Component {
  ...
  renderMenus(items) {
    return items.map(item => {
      if (item.children && item.children.length > 0) {
        ...
        const subMenuCls = containSelected ? 'jgui-dropdown-submenu-contain-selected' : '';
        return (
  ...
  render() {
    ...
    const menus = column.filterDropdown ? column.filterDropdown : (
      <div className="jgui-table-filter-dropdown">
        <Menu
          multiple={multiple}
          onClick={this.handleMenuItemClick}
          prefixCls="jgui-dropdown-menu"
        ...
        <div className="jgui-table-filter-dropdown-btns">
          <a
            className="jgui-table-filter-dropdown-link confirm"
            onClick={this.handleConfirm}
          >
            {locale.filterConfirm}
          </a>
          <a
            className="jgui-table-filter-dropdown-link clear"
